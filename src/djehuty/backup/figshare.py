...
class FigshareEndpoint:
    ...
    def __init__(self, maximum_simultaneous_connections=None):
        ...
        adapter = requests.adapters.HTTPAdapter(
            pool_connections=2, pool_maxsize=maximum_simultaneous_connections, pool_block=False
        )
    ...
    def get_statistics(self, path: str, headers, parameters):
        ...
        response = self.requests.get(f"{self.stats_base}{path}", headers=headers, params=parameters)
    ...
    def get(self, path: str, headers, parameters):
        """Procedure to perform a GET request to a Figshare-compatible endpoint."""
        try:
            response = self.requests.get(self.base + path, headers=headers, params=parameters)
            if response.status_code == 200:
                return response.json()
    ...
    def get_datasets(self, published_since="1970-01-01", published_until="2300-01-01"):
        """Procedure to get datasets from the Figshare API."""

        return self.get_all(
            "/articles", published_since=published_since, published_until=published_until
        )
    ...
    def get_dataset_details_by_account_by_id(self, account_id, account_uri, dataset_id):
        ...
        record["private_links"] = self.get_dataset_private_links_by_account_by_id(
        ...
        if conv.value_or(record, "is_public", False):
            versions = self.get_dataset_versions(dataset_id, account_id, latest=current_version)
        ...
        if conv.value_or(record, "is_public", False):
            ...
            if "created_date" in record and not record["created_date"] is None:
                date = datetime.strptime(
                    record["created_date"].replace("Z", "").replace("T", " "), "%Y-%m-%d %H:%M:%S"
                )
                created_date = datetime.strftime(date, "%Y-%m-%d")

            record["statistics"] = self.get_statistics_for_dataset(dataset_id, created_date, now)
    ...
    def get_dataset_private_links_by_account_by_id(self, account_id, dataset_id):
        ...
        headers = self.__request_headers()
        parameters = {"impersonate": account_id}
        record = self.get(f"/account/articles/{dataset_id}/private_links", headers, parameters)
        return record
    ...
    def get_collection_private_links_by_account_by_id(self, account_id, collection_id):
        ...
        parameters = {"impersonate": account_id}
        record = self.get(
            f"/account/collections/{collection_id}/private_links", headers, parameters
        )
        return record
    ...
    def get_datasets_by_account(self, account_id, account_uri):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            datasets = [
                executor.submit(
                    self.get_dataset_details_by_account_by_id, account_id, account_uri, dataset_id
                )
            ...
            logging.debug(
                "Fetched %d full datasets in %.2f seconds", total_fetched, end_time - start_time
            )
            return results
    ...
    def get_collections_by_account(self, account_id, account_uri):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            start_time = time.perf_counter()
            collections = [
                executor.submit(self.get_collection_by_id, account_id, account_uri, collection_id)
            ...
            total_fetched = len(results)
            logging.debug(
                "Fetched %d full collections in %.2f seconds", total_fetched, end_time - start_time
            )
            return results
    ...
    def get_collection_by_id(self, account_id, account_uri, collection_id):
        ...
        headers = self.__request_headers()
        parameters = {"impersonate": account_id}
        record = self.get(f"/account/collections/{collection_id}", headers, parameters)
        datasets = self.get_datasets_for_collection(account_id, collection_id)
        authors = []
        for author in record["authors"]:
            details = self.get_author_details_by_id(author["id"], account_id)
            authors.append(details)

        private_links = self.get_collection_private_links_by_account_by_id(
        ...
        if conv.value_or(record, "public", False):
            ...
            for number_record in numbers:
                ...
                versioned_record = self.get_record(
                    f"/collections/{collection_id}/versions/{number}"
                )
                versioned_record["is_latest"] = 0
                versioned_record["is_editable"] = 0
                version = conv.value_or_none(versioned_record, "version")
                if current_version is not None and version == current_version:
                    datasets = self.get_datasets_for_collection(
                        account_id, collection_id, is_public=True
                    )
        ...
        record["statistics"] = self.get_statistics_for_collection(
            collection_id, None, datetime.strftime(datetime.now(), "%Y-%m-%d")
        )
    ...
    def get_dataset_versions(self, dataset_id, account_id, exclude=None, latest=None):
        ...
        for item in versions:
            ...
            if exclude is None or version != exclude:
                ...
                record["review"] = next(
                    (item for item in reviews if item["version"] == int(version)), None
                )
    ...
    def get_institutional_accounts(self, account_id=None):
        ...
        return self.get(
            ...
            {"id_lte": account_id, "id_gte": account_id},
        )
    ...
    def get_dataset_reviews(self, dataset_id):
        ...
        return self.get(
            ...
            self.__request_headers(),
            {"article_id": dataset_id, "limit": 10},
        )

    def get_author_details_by_id(self, author_id, account_id):
        """Procedure to get a detailed author record."""

        return self.get_record(f"/account/authors/{author_id}", impersonate=account_id)

    def get_statistics_for_type(self, item_id, item_type, start_date=None, end_date=None):
        ...
        try:
            output = {"views": None, "downloads": None, "shares": None, "totals": totals}
        except KeyError:
            logging.error("Failed to gather statistics for %s %d.", item_type, item_id)
            output = {"views": None, "downloads": None, "shares": None, "totals": None}

        return output

    def get_statistics_for_dataset(self, dataset_id, start_date=None, end_date=None):
        """Procedure to get statistics for a dataset."""
        return self.get_statistics_for_type(
            item_id=dataset_id, item_type="article", start_date=start_date, end_date=end_date
        )

    def get_statistics_for_collection(self, collection_id, start_date=None, end_date=None):
        """Procedure to get statistics for a collection."""
        return self.get_statistics_for_type(
            item_id=collection_id, item_type="collection", start_date=start_date, end_date=end_date
        )
