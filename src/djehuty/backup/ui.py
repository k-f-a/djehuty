...
def process_datasets_for_account(endpoint, account):
    ...
    datasets = endpoint.get_datasets_by_account(account["id"], account["uri"])
    number_of_datasets = len(datasets)
    for dataset_index, dataset in enumerate(datasets):
        logging.info("Processing dataset %d of %d.", dataset_index + 1, number_of_datasets)
...
def process_collections_for_account(endpoint, account):
    ...
    collections = endpoint.get_collections_by_account(account["id"], account["uri"])
    number_of_collections = len(collections)
    for collection_index, collection in enumerate(collections):
        logging.info("Processing collection %d of %d.", collection_index + 1, number_of_collections)
        ...
        try:
            if not (versions and versions[-1]["modified_date"] == collection["modified_date"]):
                if endpoint.rdf_store.insert_collection(collection, account["id"], account["uri"]):
        ...
        for version in versions:
            if not endpoint.rdf_store.insert_collection(version, account["id"], account["uri"]):
...
def main(figshare_token, figshare_stats_auth, account_id, api_url):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as runner:
        results = [
            runner.submit(process_datasets_for_account, endpoint, account) for account in accounts
        ]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as runner:
        results = [
            runner.submit(process_collections_for_account, endpoint, account)
            for account in accounts
        ]
        for response in results:
            result = response.result()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as runner:
        results = [
            runner.submit(process_author_links_for_account, endpoint, account)
            for account in accounts
        ]
        for response in results:
            result = response.result()
