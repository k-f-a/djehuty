...
def rdflib_network_audit_hook(name, arguments):
    ...
    if name == "urllib.Request" and arguments[0] not in (
        f"{config.endpoint}?",
...
class SparqlInterface:
    ...
    def __init__(self):
        ...
        self.jinja = Environment(
            loader=FileSystemLoader(
                os.path.join(os.path.dirname(__file__), "resources", "sparql_templates")
            ),
            autoescape=True,
        )
        self.sparql = None
    ...
    def setup_sparql_endpoint(self):
        ...
        # is enabled.
        elif isinstance(config.endpoint, str) and config.endpoint.startswith("memory://"):
        ...
        try:
            ...
            if execution_type == "update":
                ...
                if query_type == "ASK":
                    ...
                    self.log.error(
                        "Error executing query (%s): %s", query_results[0], query_results[1]
                    )
                    self.__log_query(query)
                    return []
                else:
                    results = list(map(self.__normalize_binding, query_results.bindings))
            else:
                ...
                if error.code == 503:
                    if retries > 0:
                        ...
                        return self.__run_query(
                            query,
                            cache_key_string=cache_key_string,
                            prefix=prefix,
                            retries=retries - 1,
                        )
                    self.log.warning("Giving up on retrying SPARQL request.")
                self.log.error(
                    "SPARQL endpoint returned %d:\n---\n%s\n---", error.code, error.reason
                )
                return []
    ...
    def dataset_storage_used(self, dataset_uuid):
        ...
        query = self.__query_from_template("dataset_storage_used", {"dataset_uuid": dataset_uuid})
    ...
    def dataset_versions(
        self, limit=1000, offset=0, order="version", order_direction="desc", container_uri=None
    ):
        """Procedure to retrieve the versions of a dataset."""

        query = self.__query_from_template("dataset_versions", {"container_uri": container_uri})
    ...
    def container_items(
        ...
        query = self.__query_from_template(
            "container_items",
            {
    ...
    def __search_query_to_sparql_filters_v2(self, search_for, search_format):
        ...
        if dict in list(map(type, search_for)):
            ...
            for search_term in search_for:
                ...
                for field in fields:
                    filter_list.append(
                        f"       CONTAINS(LCASE(?{field}),          {search_term_safe})"
                    )

                if search_format:
                    filter_list.append(
                        f"       CONTAINS(LCASE(?format),         {search_term_safe})"
                    )
    ...
    def __search_query_to_sparql_filters_v3(self, search_query):
        ...
        for scope in scopes:
            ...
            for word in search_for:
                filters += "("
                filters += rdf.sparql_contains_filter(
                    scope, word, andgate=andgate, decapsulate=True
                )
                filters += " ) && "
            filters = filters[:-4] + " ) || "
        filters = filters[:-4] + " )\n"
    ...
    def datasets(
        ...
        filters = rdf.sparql_filter(
            "container_uri", rdf.uuid_to_uri(container_uuid, "container"), is_uri=True
        )
        filters += rdf.sparql_filter(
            "dataset", rdf.uuid_to_uri(dataset_uuid, "dataset"), is_uri=True
        )
        ...
        filters += rdf.sparql_filter(
            "codecheck_certificate_doi", codecheck_certificate_doi, escape=True
        )
    ...
    def repository_file_statistics(self, extended_properties=False, use_cache=False):
        ...
        query = self.__query_from_template(
            "statistics_files", {"extended_properties": extended_properties}
        )
    ...
    def repository_datalink_statistics(self, use_cache=False):
        ...
        query = self.__query_from_template("statistics_datalink", {"filters": filters})
    ...
    def repository_statistics(self):
        ...
        try:
            ...
            collections = self.__run_query(
                collections_query, collections_query, "repository_statistics"
            )
            ...
            files_results = {"files": number_of_files, "bytes": number_of_bytes}
    ...
    def dataset_statistics_timeline(
        self,
        dataset_id=None,
        item_type="downloads",
        order="downloads",
    ...
    def container_uuid_by_id(self, identifier, item_type="dataset"):
        ...
        if conv.parses_to_int(identifier):
            try:
                query = self.__query_from_template(
                    "container_uuid_by_id", {"container_id": identifier, "item_type": item_type}
                )
    ...
    def container(self, container_uuid, item_type="dataset", use_cache=True):
        ...
        query = self.__query_from_template(
            "container", {"item_type": item_type, "container_uuid": container_uuid}
        )
    ...
    def authors(
        ...
        if search_for is not None:
            ...
            filters += (
                f"FILTER (CONTAINS(LCASE(?first_name), {escaped}) ||\n"
    ...
    def author_profile(self, author_uri):
        ...
        query = self.__query_from_template("author_profile", {"author_uri": author_uri})
        return self.__run_query(query)

    def author_public_items(self, author_uri):
        """Returns the public datasets and collections of a given AUTHOR_URI."""
        query = self.__query_from_template("author_public_items", {"author_uri": author_uri})
        return self.__run_query(query)

    def associated_authors(self, author_uri):
        """Returns collaborating authors for a given AUTHOR_URI."""
        query = self.__query_from_template("associated_authors", {"author_uri": author_uri})
        return self.__run_query(query)
    ...
    def derived_from(
        self, item_uri, item_type="dataset", order=None, order_direction=None, limit=10
    ):
        """Procedure to retrieve derived_from links"""

        query = self.__query_from_template(
            "derived_from", {"prefix": item_type.capitalize(), "item_uri": item_uri}
        )
    ...
    def custom_fields(
        ...
        query = self.__query_from_template(
            "custom_fields",
            {"item_uri": item_uri, "item_type": item_type, "prefix": prefix, "filters": filters},
        )
        query += rdf.sparql_suffix(order, order_direction, limit, None)
        ...
        if derived_from_items:
            results.append({"name": "Derived From", "value": derived_from_items})
        return results
    ...
    def tags(self, order=None, order_direction=None, limit=10, item_uri=None, account_uuid=None):
        """Procedure to get tags for a dataset or a collection."""

        query = self.__query_from_template(
            "tags", {"account_uuid": account_uuid, "item_uri": item_uri}
        )
    ...
    def account_categories(
        self, account_uuid, title=None, order=None, order_direction=None, limit=10
    ):
        """Procedure to retrieve categories of a dataset."""

        filters = rdf.sparql_filter("title", title, escape=True)
        query = self.__query_from_template(
            "account_categories", {"account_uuid": account_uuid, "filters": filters}
        )
        query += rdf.sparql_suffix(order, order_direction, limit, None)
    ...
    def license_url_by_id(self, license_id):
        ...
        query = self.__query_from_template("licenses", {"license_id": license_id})
    ...
    def latest_datasets_portal(self, page_size=30):
        ...
        query = self.__query_from_template("latest_datasets_portal", {"page_size": page_size})
    ...
    def collections_from_dataset(self, dataset_container_uuid):
        ...
        query = self.__query_from_template(
            "collections_from_dataset", {"dataset_container_uuid": dataset_container_uuid}
        )
    ...
    def collection_datasets(self, collection_uri, limit=None, offset=0):
        ...
        query = self.__query_from_template("collection_datasets", {"collection": collection_uri})
    ...
    def contact_info_from_container(self, container_uuid):
        ...
        query = self.__query_from_template(
            "contact_info_from_container", {"container_uuid": container_uuid}
        )
        try:
            return self.__run_query(query)[0]
        except (TypeError, IndexError):
            self.log.error("Retrieving contact info for container %s failed.", container_uuid)
            return None
    ...
    def collection_versions(
        ...
        query = self.__query_from_template("collection_versions", {"filters": filters})
        query += rdf.sparql_suffix(order, order_direction, limit, offset)
        return self.__run_query(query)
    ...
    def collections_dataset_count(self, collection_uri):
        ...
        query = self.__query_from_template(
            "collection_datasets_count", {"collection_uri": collection_uri}
        )
        results = self.__run_query(query)
    ...
    def collections(
        ...
        filters += rdf.sparql_filter(
            "collection", rdf.uuid_to_uri(collection_uuid, "collection"), is_uri=True
        )
        filters += rdf.sparql_filter("institution_id", institution)
        ...
        if search_for is not None:
            ...
            filters += (
                f"FILTER (CONTAINS(STR(?title),          {escaped}) ||\n"
        ...
        if use_cache:
            cache_prefix = (
                f"collections_{account_uuid}" if account_uuid is not None else "collections"
            )
            return self.__run_query(query, query, cache_prefix)

        return self.__run_query(query)

    def collections_by_account(
        self, account_uuid=None, limit=100, offset=None, order=None, order_direction=None
    ):
        """Procedure to retrieve essential metadata of collections of an account."""

        query = self.__query_from_template("collections_by_account", {"account_uuid": account_uuid})
        query += rdf.sparql_suffix(order, order_direction, limit, offset)
    ...
    def fundings(
        ...
        if search_for is not None:
            escaped = rdf.escape_string_value(search_for.lower())
            filters += (
                f"FILTER (CONTAINS(LCASE(?title),       {escaped}) ||\n"
    ...
    def references(
        self, order=None, order_direction=None, limit=10, item_uri=None, account_uuid=None
    ):
    ...
    def collection_dataset_containers(self, collection_uri, limit=10):
        ...
        query = self.__query_from_template(
            "collection_dataset_containers", {"collection_uri": collection_uri}
        )
    ...
    def update_dataset_git_uuid(self, dataset_uuid, account_uuid):
        ...
        query = self.__query_from_template(
            "update_git_uuid",
            {"dataset_uuid": dataset_uuid, "git_uuid": rdf.escape_string_value(new_git_uuid)},
        )
    ...
    def insert_dataset(
        ...
        for key in custom_fields:
            try:
                self.insert_custom_field_value(
                    name=key, value=custom_fields[key], item_uri=uri, graph=graph
                )
        ...
        rdf.add(
            graph, uri, rdf.DJHT["codecheck_certificate_doi"], codecheck_certificate_doi, XSD.string
        )
    ...
    def update_quota_request(
        self, quota_request_uuid, requested_size=None, reason=None, status=None
    ):
    ...
    def update_account(
        ...
        if results and categories:
            ...
            self.insert_item_list(
                graph, URIRef(rdf.uuid_to_uri(account_uuid, "account")), items, "categories"
            )

            if not self.add_triples_from_graph(graph):
                self.log.error("Updating categories for account %s failed.", account_uuid)
                return None
    ...
    def update_item_list(self, item_uuid, account_uuid, items, predicate):
        ...
        try:
            graph = Graph()
            item = self.container_items(
                item_uuid=item_uuid, is_published=None, is_latest=None, account_uuid=account_uuid
            )[0]

            self.delete_associations(item_uuid, account_uuid, predicate)
            if items:
                self.insert_item_list(graph, URIRef(item["uri"]), items, predicate)

                if not self.add_triples_from_graph(graph):
                    self.log.error("%s insert query failed for %s", predicate, item_uuid)
                    return False

            return True

        except IndexError:
            self.log.error("Could not insert %s items for %s", predicate, item_uuid)
    ...
    def insert_author(
        ...
        rdf.add(
            graph, author_uri, rdf.DJHT["created_by"], rdf.uuid_to_uri(created_by, "account"), "uri"
        )
    ...
    def delete_item_from_list(self, subject, predicate, rdf_first_value, value_type="uri"):
        ...
        query = self.__query_from_template(
            ...
            {"subject": subject, "predicate": rdf.DJHT[predicate].n3(), "first": first},
        )
    ...
    def __append_to_existing_list(self, base_item_uri, existing_items):
        ...
        query = self.__query_from_template(
            "append_to_list", {"last_blank_node": last_node, "append_blank_node": new_node}
        )
    ...
    def insert_file(
        ...
        if self.add_triples_from_graph(graph):
            ...
            existing_files = self.dataset_files(
                dataset_uri=dataset_uri, limit=None, account_uuid=account_uuid
            )
    ...
    def insert_log_entry(
        self, created_date, ip_address, item_uuid, item_type="dataset", event_type="view"
    ):
    ...
    def insert_group_member(self, group_uuid, account_uuid, is_supervisor):
        ...
        if not existing_members:
            self.insert_item_list(
                graph, URIRef(rdf.uuid_to_uri(group_uuid, "group")), [URIRef(member_uri)], "members"
            )
        if self.add_triples_from_graph(graph):
            if existing_members:
    ...
    def item_collaborative_permissions(self, item_type, item_uuid, collaborator_account_uuid):
        """
        ...
        query = self.__query_from_template(
            "item_collaborative_permissions",
            {
                "item_type": item_type,
                "item_uuid": item_uuid,
    ...
    def insert_collaborator(
        ...
        rdf.add(
            ...
            collaborator_uri,
            rdf.DJHT["item"],
            rdf.uuid_to_uri(dataset_uuid, "dataset"),
            "uri",
        )
        rdf.add(
            graph,
            collaborator_uri,
            rdf.DJHT["account"],
            rdf.uuid_to_uri(collaborator_uuid, "account"),
            "uri",
        )
        rdf.add(
            graph,
            collaborator_uri,
            rdf.DJHT["added_by"],
            rdf.uuid_to_uri(account_uuid, "account"),
            "uri",
        )
    ...
    def delete_collaborator(self, dataset_uuid, collaborator_uuid):
        ...
        query = self.__query_from_template(
            "delete_collaborator",
            {"dataset_uuid": dataset_uuid, "collaborator_uuid": collaborator_uuid},
        )
    ...
    def insert_private_link(
        ...
        if self.add_triples_from_graph(graph):
            ...
            if item_type == "dataset":
                ...
                item = self.collections(
                    collection_uuid=item_uuid,
                    account_uuid=account_uuid,
                    # See above.
                    is_published=None,
                    is_latest=None,
            ...
            if item is None:
                self.log.error("Could not find item to insert a private link %s for.", link_uri)
                return None

            if self.update_item_list(
                item["uuid"], account_uuid, [URIRef(link_uri)], "private_links"
            ):
                return link_uri

        return None

    def insert_custom_field_value(self, name=None, value=None, item_uri=None, graph=None):
        ...
        if name.lower() not in allowed_custom_fields:
            self.log.warning("Blocked inserting custom field '%s' for <%s>.", name, item_uri)
            return False
    ...
    def dataset_is_under_review(self, dataset_uuid):
        ...
        """
        query = self.__query_from_template(
            "dataset_is_under_review", {"dataset_uuid": dataset_uuid}
        )
    ...
    def publish_collection(self, container_uuid, account_uuid):
        ...
        draft = None
        try:
            draft = self.collections(
                container_uuid=container_uuid, is_published=False, use_cache=False
            )[0]
        except IndexError:
            self.log.error("Attempted to publish without a draft <container:%s>.", container_uuid)
            return False

        new_version_number = 1
        latest = None
        try:
            latest = self.collections(
                container_uuid=container_uuid, is_published=True, is_latest=True, use_cache=False
            )[0]
    ...
    def create_draft_from_published_collection(self, container_uuid):
        ...
        draft_derived_from = self.derived_from(
            item_uri=latest_uri, item_type="collection", limit=None
        )
        ...
        draft_dataset_uris = list(
            {URIRef(container["container_uri"]) for container in draft_datasets}
        )
    ...
    def decline_dataset(self, container_uuid, account_uuid):
        ...
        try:
            self.datasets(container_uuid=container_uuid, is_published=False)[0]
        except IndexError:
            self.log.error("Attempted to decline without a draft <container:%s>.", container_uuid)
            return False

        query = self.__query_from_template(
            "decline_draft_dataset",
            {
                "container_uuid": container_uuid,
            },
        )
    ...
    def create_draft_from_published_dataset(self, container_uuid, account_uuid=None):
        ...
        draft_files = self.dataset_files(
            dataset_uri=latest_uri, account_uuid=account_uuid, limit=None
        )
        draft_tags = self.tags(item_uri=latest_uri, limit=None)
        draft_categories = self.categories(item_uri=latest_uri, limit=None)
        draft_references = self.references(item_uri=latest_uri, limit=None)
    ...
    def update_dataset(
        ...
        mimetype=None,
        contributors=None,
        license_remarks=None,
        geolocation=None,
        longitude=None,
        latitude=None,
        data_link=None,
        has_linked_file=None,
        derived_from=None,
        same_as=None,
        organizations=None,
        categories=None,
        ...
        is_restricted=False,
        references=None,
        agreed_to_deposit_agreement=False,
        agreed_to_publish=False,
        ...
        git_code_hosting_url=None,
        funding_list=None,
        requested_codecheck=False,
        codecheck_certificate_doi=None,
    ):
        ...
        query = self.__query_from_template(
            "update_dataset",
            {
                ...
                "agreed_to_deposit_agreement": rdf.escape_boolean_value(
                ...
                "codecheck_certificate_doi": rdf.escape_string_value(
                    conv.normalize_doi(codecheck_certificate_doi)
                ),
            },
        )
    ...
    def delete_dataset_embargo(self, dataset_uri, account_uuid):
        ...
        query = self.__query_from_template(
            "delete_dataset_embargo", {"account_uuid": account_uuid, "dataset_uri": dataset_uri}
        )
    ...
    def delete_private_links(self, container_uuid, account_uuid, link_id):
        ...
        query = self.__query_from_template(
            "delete_private_links",
            {"account_uuid": account_uuid, "container_uuid": container_uuid, "id_string": link_id},
        )
    ...
    def update_private_link(
        self, item_uri, account_uuid, link_id, is_active=None, expires_date=None, read_only=None
    ):
    ...
    def dataset_update_thumb(self, dataset_uuid, account_uuid, file_uuid, extension, version=None):
    ...
    def update_doi_after_publishing(self, item_uuid, item_type, doi):
        ...
        query = self.__query_from_template(
            "update_doi_after_publishing",
            {"item_uuid": item_uuid, "item_type": item_type, "doi": rdf.escape_string_value(doi)},
        )
        return self.__run_logged_query(query)
    ...
    def dataset_update_seen_by_reviewer(self, dataset_uuid):
        ...
        current_time = datetime.strftime(datetime.now(), datetime_format)
        query = self.__query_from_template(
            "update_seen_by_reviewer", {"timestamp": current_time, "dataset_uuid": dataset_uuid}
        )
        return self.__run_logged_query(query)
    ...
    def category_by_id(self, category_id=None, category_uuid=None):
        ...
        query = self.__query_from_template(
            "category_by_id", {"category_id": category_id, "category_uuid": category_uuid}
        )
    ...
    def root_categories(self):
        ...
        query = self.__query_from_template("root_categories", {"state_graph": config.state_graph})
    ...
    def categories_tree(self):
        ...
        for root in roots:
            ...
            subcategories = filter(
                lambda category: conv.value_or_none(category, "parent_id") == root["id"], results
    ...
    def group(
        ...
        query = self.__query_from_template(
            "group", {"account_uuid": account_uuid, "filters": filters}
        )
    ...
    def group_by_name(self, group_name, startswith=False):
        ...
        query = self.__query_from_template(
            "group_by_name",
            {"startswith": startswith, "group_name": rdf.escape_string_value(group_name)},
        )
    ...
    def account_storage_used(self, account_uuid):
        ...
        query = self.__query_from_template("account_storage_used", {"account_uuid": account_uuid})
    ...
    def opendap_to_doi(self, startswith=None, endswith=None):
        ...
        query = self.__query_from_template("opendap_to_doi", {"filters": filters})
    ...
    def insert_review(
        self, dataset_uri, request_date=None, assigned_to=None, status=None, reminder_date=None
    ):
    ...
    def account_uuid_by_orcid(self, orcid):
        ...
        query = self.__query_from_template(
            "account_uuid_by_orcid", {"orcid": rdf.escape_string_value(orcid)}
        )
    ...
    def account_by_session_token(self, session_token, mfa_token=None):
        ...
        query = self.__query_from_template(
            "account_by_session_token",
            {"token": rdf.escape_string_value(session_token), "mfa_token": mfa_token},
        )
    ...
    def account_by_email(self, email):
        ...
        query = self.__query_from_template(
            "account_by_email", {"email": rdf.escape_string_value(email)}
        )
    ...
    def missing_checksummed_files_for_container(self, container_uuid):
        ...
        query = self.__query_from_template(
            "missing_checksummed_files_for_container", {"container_uuid": container_uuid}
        )
        return self.__run_query(query)
    ...
    def initialize_privileged_accounts(self):
        ...
        for email in privileged_accounts:
            ...
            account_uuid = self.insert_account(
                email=email, orcid_id=orcid, first_name=first_name, last_name=last_name
            )
            if not account_uuid:
    ...
    def insert_session(
        self, account_uuid, name=None, token=None, editable=False, override_mfa=False
    ):
    ...
    def delete_inactive_session_by_uuid(self, session_uuid):
        ...
        query = self.__query_from_template(
            "delete_inactive_session_by_uuid", {"session_uuid": session_uuid}
        )
    ...
    def delete_session_by_uuid(self, account_uuid, session_uuid):
        ...
        query = self.__query_from_template(
            "delete_session_by_uuid", {"session_uuid": session_uuid, "account_uuid": account_uuid}
        )
    ...
    def delete_session(self, token):
        ...
        query = self.__query_from_template(
            "delete_session", {"token": rdf.escape_string_value(token)}
        )
        return self.__run_logged_query(query)
    ...
    def sessions(self, account_uuid, session_uuid=None, mfa_token=None):
        ...
        query = self.__query_from_template(
            "account_sessions",
            {"account_uuid": account_uuid, "session_uuid": session_uuid, "mfa_token": mfa_token},
        )
    ...
    def may_query(self, session_token, account=None):
        ...
        return self.__may_execute_role(
            session_token, "administer", account
        ) and self.__may_execute_role(session_token, "query", account)
    ...
    def may_review_integrity(self, session_token, account=None):
        ...
        return self.__may_execute_role(
            session_token, "administer", account
        ) and self.__may_execute_role(session_token, "review_integrity", account)
    ...
    def properties_for_type(self, rdf_type):
        ...
        query = self.__query_from_template("explorer_properties", {"type": rdf_type})
    ...
    def types_for_property(self, rdf_type, rdf_property):
        ...
        query = self.__query_from_template(
            "explorer_property_types", {"type": rdf_type, "property": rdf_property}
        )
    ...
    def mark_state_graph_as_initialized(self):
        ...
        query = (
            f"INSERT {{ GRAPH <{config.state_graph}> {{ <this> "
    ...
    def state_graph_is_initialized(self):
        ...
        query = (
            f"ASK {{ GRAPH <{config.state_graph}> {{ <this> "
            f'<{rdf.DJHT["initialized"]}> "true"^^<{XSD.boolean}> }} }}'
        )
        return self.__run_query(query)
    ...
    def __export_rdf_to_file(self, query, export_directory, filename):
        ...
        stream_fd = os.open(
            os.path.join(export_directory, filename), os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600
        )
        ...
        with open(stream_fd, "w", encoding="utf-8") as stream:
            for triple in records:
                ...
                stream.write(
                    f"<{triple['subject']}> <{triple['predicate']}> {value} <{config.state_graph}> .\n"
                )
            if os.name != "nt":
    ...
    def export_rdf(self, full_export=False):
        ...
        if full_export:
            root_types += ["LogEntry", "LogEntryType", "PrivateLink", "QuotaRequest", "Session"]

        for record_type in root_types:
            query = self.__query_from_template(
                "export_root_type", {"full_export": full_export, "type_name": record_type}
            )
        ...
        for record_type in listable_types:
            query = self.__query_from_template(
                "export_listable_type", {"full_export": full_export, "predicate": record_type}
            )
            self.__export_rdf_to_file(query, export_directory, f"{record_type}.n3")
